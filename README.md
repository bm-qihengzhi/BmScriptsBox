...
| 特性       | 功能说明                                                    |
| -------- | ------------------------------------------------------- |
| **开箱即用** | 搭载可视化图形操作界面，所有脚本支持一键安装、一键运行，无需手动搭建、配置运行环境，新手零基础直接使用。    |
| **优质脚本库** | 内置精选脚本库，全面覆盖日常办公、数据处理、文件操作、系统运维等全品类需求，支持关键词快速检索，一键部署启用。 |
| **声明式接入** | 无需集成基类、无需安装SDK，仅通过一个TOML配置文件，即可快速将自定义脚本接入脚本盒子，接入流程极简高效。 |

### ⚡ 高效触发

| 特性        | 功能说明                                                          |
|-----------| ------------------------------------------------------------- |
| **全局快捷键** | 支持为任意脚本绑定全局热键，随时随地通过快捷键一键唤起执行，可自动带入选中内容作为脚本入参，操作高效无卡顿。        |
| **系统右键**  | 脚本深度集成系统右键菜单，选中文件/文件夹后右键即可一键启动对应脚本，自动携带文件路径入参，省去手动复制路径步骤。     |
| **超级复制**  | 独创快捷触发方式，选中文本后双击 `Ctrl+C+C`，可一键将剪贴板内容自动作为入参拉起对应脚本，适配高频文本处理场景。 |
| **定时任务**  | 内置高性能定时任务引擎，支持自定义时分、每日/每周/每月等循环周期，脚本可全自动定时运行，实现无人值守自动化作业。     |
...
| **智能依赖管理** | 自动识别脚本所需的运行依赖与环境版本，全自动完成依赖下载、安装、部署，同时智能清理冗余依赖，无需手动运维。         |
| **隔离运行环境** | 为每一个脚本配备独立虚拟运行环境，彻底隔离不同脚本的版本冲突、依赖冲突，保障脚本运行安全、稳定、互不干扰。         |
...
| **CLI工具复用**    | 统一下载托管 FFmpeg、Pandoc 等常用命令行工具，通过Shim挂载与PATH统一配置，一次下载、多项目复用         |
| **AI模型复用**     | 统一下载托管 AI 模型，通过api/model/path 获取模型路径、多脚本模型共用                       |
