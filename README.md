...
## 功能特性
### 🚀 零门槛上手，开箱即用无负担
> 极简可视化操作模式，零环境搭建、零代码基础、零复杂配置，大幅降低自动化使用门槛，新手可快速上手，轻松落地各类自动化场景
* **开箱即用**：可视化图形操作界面，支持脚本一键安装、一键运行，无需手动配置环境，零基础零学习成本。
  
* **优质脚本库**：内置全场景精选脚本资源，覆盖办公、数据、文件、运维等场景，支持关键词检索、一键部署启用。

* **极简声明式接入**：无需SDK与基类集成，仅通过TOML配置文件即可快速接入自定义脚本，极简高效、低成本拓展。

### ⚡ 多模式高效触发，全场景极速响应
> 支持多维度脚本触发方式，适配全场景使用需求，告别繁琐手动操作，一键极速唤起脚本
* **全局快捷键**：支持自定义全局热键，任意场景一键唤起脚本，自动携带选中路径入参，操作流畅高效。
  
* **系统右键联动**：深度适配系统右键菜单，选中文件/文件夹即可一键运行脚本，自动携带路径入参，省去手动复制步骤。
  
* **超级复制唤起**：独创 `Ctrl+C+C` 双击触发，自动读取剪贴板内容入参，适配高频文本处理场景。
  
* **全自动定时任务**：内置高性能定时引擎，支持定点、日/周/月循环运行，实现无人值守常态化自动化作业。

### 🛠️ 全方位稳定适配，安全隔离运行
> 多语言全面兼容，搭配智能依赖运维与隔离运行机制，彻底解决环境报错、版本冲突等问题，保障脚本安全稳定运行。
* **全跨语言兼容**：支持 Python、JS、PowerShell、Bat、AHK、EXE、HTML 等多类脚本与程序，适配绝大多数自动化场景。
  
* **智能依赖托管**：自动识别、下载、安装脚本所需依赖，智能清理冗余资源，全程无需人工运维。
  
* **独立隔离运行环境**：为每个脚本分配专属虚拟运行环境，脚本之间互不干扰，彻底杜绝依赖、版本冲突问题。

### 💾 精细化资源优化，轻量高效低消耗
> 通过资源复用、智能适配、网络加速多重优化，减少设备资源冗余，降低内存与磁盘消耗，实现轻量化、高效率运行体验。

* **依赖全局缓存复用**：采用硬链接缓存机制，多脚本共享Python、Node\.js依赖，避免重复下载，节省磁盘空间、提升加载速度。
  
* **CLI工具统一复用**：统一托管FFmpeg、Pandoc等开源命令行工具，一次下载、多脚本共用，减少冗余存储。
  
* **AI模型共享托管**：统一托管huggingface、modelscope AI模型，一次下载、多脚本共用，减少冗余存储。
  
* **智能环境版本匹配**：自动识别脚本适配版本，精准匹配最优运行环境，无需手动切换配置。
  
* **GitHub高速下载**：加速链路优化海外资源下载，解决下载慢、中断、失败问题，提升部署效率。
