...
def local_test():
    ...
    # 模拟 Manifest 数据
    # with open(r'D:\PythonCode\BmScriptsBox-Code\BmPackages\BmPackage.json', 'r', encoding='utf-8') as f:
    #     import json
    #     manifest = json.load(f)
    manifest = {
        "node": {
            "versions": {
                "v22.16.0": {"path": "/path/to/node18"}
            }
        },
        "python": {
            "versions": {
                "3.8.10": {"path": "/path/to/node19"},
                "3.13.9": {"path": "/path/to/node18"}
            }
        },
        "autohotkey": {
            "versions": {
                "1.1.37.02": {"path": "/path/to/node19"},
                "2.0.19": {"path": "/path/to/node18"}
            }
        },
    }

    # 测试匹配
    res = matcher.find_best_local_version(manifest, "node", ">=v19.9")
    print(res)
    res2 = matcher.find_best_local_version(manifest, "python", ">=3.9")
    print(res2)
    res3 = matcher.find_best_local_version(manifest, "autohotkey", ">=3.0")
    print(res3)
...
def cloud_test():
    import requests
    url = f"https://gitee.com/bmscriptsbox/bm-binary-resource/raw/master/node.json"
    response = requests.get(url, timeout=10)
    response.raise_for_status()
    cloud_data = response.json()
    matcher = VersionMatcher()
    res = matcher.fetch_best_cloud_version(cloud_data, "node", ">=18.5")
    print(res)
